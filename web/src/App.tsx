...
import { createPhpAssetLoader } from "./phpAssetLoader";
...
const phpAssetLoader = createPhpAssetLoader(apiBase, apiFetch);
...
export default function App() {
  ...
  const spineThickness = useMemo(
    () => spineThicknessMm(innerPages, grammage),
    [innerPages, grammage],
  );
  ...
  async function ensureWasmCompiler(): Promise<TypstCompiler> {
    ...
    try {
      const compiler = await createTypstCompiler({
        getTypstWasmBuffer: () => fetchCachedArrayBuffer(typstCompilerWasmUrl),
        loader: phpAssetLoader,
        onFontProgress: (loaded, total) =>
          setProgress({
            phase: `Polices ${loaded} / ${total}`,
            ratio: loaded / Math.max(total, 1),
          }),
      });
      compilerRef.current = compiler;
      setWasmReady(true);
      pushLog("Typst compiler init OK.");
      return compiler;
    } finally {
      setProgress(null);
    }
  ...
  async function compileTypstToPdfWasm() {
    ...
    try {
      ...
      const out = await compileTypstBookToPdf({
        compiler,
        loader: phpAssetLoader,
        templateMainSource: String(tplData.source),
        generatedTypst,
        bookLayout,
      ...
      if (!out.pdf) {
        setRenderLog(JSON.stringify(out.diagnostics ?? [], null, 2));
        setStatus("Compilation Typst WASM sans resultat PDF.");
        pushLog("Typst world.pdf sans resultat.");
        return;
      }

      const blob = new Blob([new Uint8Array(out.pdf)], { type: "application/pdf" });
      const file = new File([blob], "typst-wasm-output.pdf", { type: "application/pdf" });
      await handlePdfFile(file);
      setRenderLog(JSON.stringify(out.diagnostics ?? [], null, 2));
      setStatus("PDF genere en WASM.");
      pushLog(
        `Typst world.pdf OK: ${out.pdf.byteLength} bytes PDF, diags=${(Array.isArray(out.diagnostics) ? out.diagnostics : []).length}.`,
      );
    } catch (err) {
  ...
  async function compileCoverTypstToPdfWasm() {
    ...
    try {
      ...
      await mountTypstPackagesFromLoader(compiler, phpAssetLoader);
