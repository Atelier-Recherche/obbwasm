...
export function normalizeMediaLookupPath(path) {
...
export function isUnsafeTypstFilePath(path) {
    ...
    if (/(^|\/)\.\.(\/|$)/.test(s))
        return true;
    if (s.startsWith(OBB_MEDIA_PREFIX) || s.startsWith("/obb-media/"))
        return false;
    if (/^https?:\/\//i.test(s))
        return false;
    return false;
}
function mediaKeyNorm(k) {
    return normalizeMediaLookupPath(k).toLowerCase();
}
function mountVirtualBytes(merged, virtual, bytes) {
...
function indexMediaAliases(files) {
    const out = { ...files };
    for (const [k, buf] of Object.entries(files)) {
        const nk = mediaKeyNorm(k);
        if (nk && out[nk] === undefined)
            out[nk] = buf;
        const base = nk.split("/").pop();
        if (base && out[base] === undefined)
            out[base] = buf;
        const withMedia = `media/${base}`;
        if (base && out[withMedia] === undefined)
            out[withMedia] = buf;
    }
    return out;
}
...
export function findMediaBytes(files, path) {
    ...
    for (let len = parts.length; len > 0; len--) {
        ...
        for (const [k, buf] of Object.entries(files)) {
            ...
            if (knNorm === suffixLower || knNorm.endsWith("/" + suffixLower))
                return buf;
...
export async function virtualizeTypstMediaPaths(params) {
    ...
    let counter = 0;
    function assignPlaceholder(rawPath) {
        ...
        dbg?.push(`⚠ placeholder (fichier introuvable ou hors racine Typst) : ${rawPath.slice(0, 120)}${rawPath.length > 120 ? "…" : ""} → ${virtual}`);
        return virtual;
    }
    async function resolveOne(rawPath) {
        ...
        if (norm.usedPlaceholder) {
            dbg?.push(`⚠ image remplacée par placeholder (non-PNG / HTML / vide / conversion échouée) : ${rawPath.slice(0, 120)}${rawPath.length > 120 ? "…" : ""}`);
        }
