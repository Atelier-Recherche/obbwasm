...
export function collectTypstLabelIds(...sources) {
    const ids = new Set();
 ...
 */
export function stripNonBookWikiLinks(markdown) {
    return markdown.replace(/\[\[([^\]]+)\]\]/g, (full, inner, offset, whole) => {
        // Notes de bas de page Word/HTML : [[12]](#_ftn12) — pas un wikilink Obsidian.
        if (whole.slice(offset + full.length).startsWith("("))
            return full;
        const noteKey = inner.split("#")[0].split("|")[0].trim().toLowerCase();
        if (noteKey === "glossaire" || noteKey === "index")
            return full;
        if (inner.includes("|")) {
...
function stripTypstLinkMatch(full, slug, text, defined) {
    if (defined.has(slug))
        return full;
 ...
 */
function patchTypstLinksLinear(typst, defined) {
    const needle = "#link(";
    let out = "";
    let i = 0;
    while (i < typst.length) {
        ...
        if (typst.startsWith("label(", p)) {
            ...
            p++;
            const slugStart = p;
            while (p < typst.length && /[a-zA-Z0-9_.:-]/.test(typst[p]))
                p++;
            slug = typst.slice(slugStart, p);
            p = skipWs(typst, p);
            if (typst[p] !== ">") {
 ...
 */
export function patchPandocTypstBrokenLabelRefs(typst, definedLabels) {
    if (!typst)
        return typst;
...
export async function patchPandocTypstBrokenLabelRefsAsync(typst, definedLabels) {
 ...
 */
export function injectTypstHeadingLabels(typst) {
    return typst.replace(/#heading\(([^)]*)\)\[([^\]]+)\](?!\s*<)/g, (full, args, title) => {
        if (/label\s*:/.test(args))
            return full;
...
//# sourceMappingURL=pandocTypstLabels.js.map