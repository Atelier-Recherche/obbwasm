...
export function isBookOptionValueNonDefault(
  def: BookOptionDef,
...
export function countBookOptionsNonDefaultInDefs(
  defs: BookOptionDef[],
...
export function countNonEmptyStringOverrides(
  overrides: Record<string, string>,
  keys: readonly string[],
): number {
  let n = 0;
  for (const k of keys) {
    if ((overrides[k] ?? "").trim() !== "") n++;
  }
  return n;
}
