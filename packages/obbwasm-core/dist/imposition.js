...
export function parseImpositionMeta(source) {
    ...
    if (!modeRaw)
        return null;
    const mode = modeRaw;
    if (mode !== "signature-grid" && mode !== "signature-2up" && mode !== "spread-pair")
        return null;
...
function getSignatureGridSlotMap(cols, rows, packetSize) {
    ...
    if (cols === 4 && rows === 2 && packetSize === 16) {
        ...
        const face = (indices, flip, row) => indices.map((packIndex, col) => ({ xMm: col, yMm: row, packIndex, flip }));
        return {
            front: [...face(top, true, 0), ...face(bottom, false, 1)],
            back: [...face(topBack, true, 0), ...face(bottomBack, false, 1)],
        };
    }
    throw new Error(`Grille signature ${cols}x${rows} pour paquet ${packetSize} non supportee (modes generiques a etendre).`);
}
function buildSignatureGridImpositionTyp(layout, packets) {
    const cols = layout.gridCols;
    const rowGutter = parseMm(layout.gridRow2DyOffset) !== 0 ? layout.gridRow2DyOffset : "0pt";
    ...
    const gridOrder = (face) => [...face].sort((a, b) => a.yMm - b.yMm || a.xMm - b.xMm);
    const emitFace = (pack, face, alignRight) => {
...
export function buildImpositionTyp(params) {
    ...
    if (layout?.mode === "signature-grid") {
        ...
        if (gridLayout.gridCols * gridLayout.gridRows * 2 !== effectivePacket) {
            throw new Error(`Grille ${gridLayout.gridCols}x${gridLayout.gridRows} incompatible avec paquet ${effectivePacket} pages.`);
        }
