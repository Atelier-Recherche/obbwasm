...
function pageNumberPattern(style) {
    if (style === "roman")
        return "i";
    if (style === "page-n-of-m")
        return "1"; // complété par en-tête personnalisé si besoin
    return "1";
}
/** Numérotation des titres : chaîne Typst ou none. */
function headingNumberingPattern(auto) {
    return auto ? "1.1.1" : "none";
}
function typstBool(v) {
    return v ? "true" : "false";
}
function typstStr(s) {
 ...
 */
export function buildTypstOptsLines(state, resolvedDocStrings, meta) {
    const v = state.values;
    const getStr = (key, fallback = "") => String(v[key] ?? fallback);
    const getBool = (key) => v[key] === true;
...
//# sourceMappingURL=typstSerialize.js.map