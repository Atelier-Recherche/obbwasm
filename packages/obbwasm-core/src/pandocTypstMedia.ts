...
function stripEmptyTypstUrlLinks(typst: string): string {
  let s = typst;
...
export function stripBrokenMarkdownMedia(markdown: string): string {
  let s = markdown;
  s = s.replace(/!\[[^\]]*\]\(\s*\)/g, "");
  s = s.replace(/!\[\[\s*\]\]/g, "");
  s = s.replace(/\[\s*\]\(\s*\)/g, "");
 ...
 */
export function patchPandocTypstMedia(typst: string): string {
  if (!typst) return typst;
  let s = typst;

  s = s.replace(
    /image\.decode\s*\(\s*read\s*\(\s*"([^"]*)"\s*,\s*encoding:\s*none\s*\)\s*\)/gi,
    (_, path: string) => (path.trim() ? `image("${path}")` : ""),
  );
  s = s.replace(
    /image\.decode\s*\(\s*read\s*\(\s*'([^']*)'\s*,\s*encoding:\s*none\s*\)\s*\)/gi,
    (_, path: string) => (path.trim() ? `image('${path}')` : ""),
  );
  s = s.replace(/image\.decode\s*\(\s*"([^"]*)"\s*\)/gi, (_, path: string) =>
