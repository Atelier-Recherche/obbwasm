...
const BEFORE_BODY: SectionId[] = ["cover", "titleCredits", "toc"];
const AFTER_BODY: SectionId[] = ["annexes", "listFigures", "bibliography", "indexGlossary", "backCover"];

function ensureContains(order: SectionId[], id: SectionId): SectionId[] {
  if (order.includes(id)) return [...order];
  const canon = CANONICAL_SECTION_ORDER.indexOf(id);
  if (canon < 0) return [...order, id];
...
function removeId(order: SectionId[], id: SectionId): SectionId[] {
 ...
 */
export function applyTocPosition(order: SectionId[], tocPosition: "none" | "start" | "end"): SectionId[] {
  let next = removeId(order, "toc");
  if (tocPosition === "none") return next;
 ...
 */
export function applyBibliographyPosition(
  order: SectionId[],
  bibPosition: "none" | "start" | "end",
): SectionId[] {
  let next = removeId(order, "bibliography");
  if (bibPosition === "none") return next;
  const bodyIdx = next.indexOf("body");
...
export function deriveTocPosition(order: SectionId[]): "none" | "start" | "end" {
  const ti = order.indexOf("toc");
  const bi = order.indexOf("body");
  if (ti < 0) return "none";
  if (bi < 0) return "start";
  return ti < bi ? "start" : "end";
}

export function deriveBibliographyPosition(order: SectionId[]): "none" | "start" | "end" {
