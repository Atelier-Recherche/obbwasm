...
function patchHr(typst: string, mode: HrMode): string {
...
export function slugifyObsidianAnchor(raw: string): string {
  const s = raw
    .trim()
    .toLowerCase()
    .normalize("NFD")
...
function escapeTypstLabelSlug(slug: string): string {
 ...
 */
export function normalizeWikiGlossaryIndexLinks(markdown: string): string {
  let s = markdown;
  s = s.replace(RX_WIKI_GLOSS, (_m, _note: string, anchor: string, alias?: string) => {
    const a = slugifyObsidianAnchor(anchor);
    const display = (alias ?? anchor).trim();
    return `[${display}](obb-glossary:${a})`;
  });
  s = s.replace(RX_WIKI_NAME_INDEX, (_m, _note: string, anchor: string, alias?: string) => {
    const a = slugifyObsidianAnchor(anchor);
    const display = (alias ?? anchor).trim();
 ...
 */
export function patchPandocTypstObbWikiRefs(typst: string): string {
  if (!typst.includes("obb-glossary") && !typst.includes("obb-index")) return typst;
  let s = typst;
  s = s.replace(/#link\(\s*"obb-glossary:([^"]+)"\s*\)/g, (_, slug: string) => {
    const esc = escapeTypstLabelSlug(slug);
    return `#link(label("obb-gl-${esc}"))`;
  });
  s = s.replace(/#link\(\s*"obb-index:([^"]+)"\s*\)/g, (_, slug: string) => {
    const esc = escapeTypstLabelSlug(slug);
...
export function splitMarkdownH1Sections(md: string): Array<{ title: string; body: string }> {
...
function escapeTypstHeadingBracketContent(title: string): string {
  return title.replace(/\\/g, "\\").replace(/\[/g, "\\[").replace(/\]/g, "\\]");
}

async function pandocBodyToTypstFragment(
...
export async function buildObbBackMatterTypstFragment(params: {
  ...
  if (!md) return "// Vide — aucune note source ou fichier absent.\n";

  const sections = splitMarkdownH1Sections(md);
  if (sections.length === 0) return "// Vide — aucune section `# titre` trouvée.\n";
